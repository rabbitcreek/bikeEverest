...
RTC_DATA_ATTR int total = 0;
RTC_DATA_ATTR float pressureBaseline = 0.0;
#ifdef ESP32
  #define SD_CS       14
  #define SRAM_CS     32
  #define EPD_CS      15
...
void setup() {
  ...
  if(esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_TIMER) tell = 1;
  print_wakeup_reason();
  if (!bmp.begin()) {
...
void printOut(){
  EEPROMWritelong( address,  total);
  int bing = random(1,11);
  if(bing == 6 ) bing =  bing + 1;
  String indexString = String(bing);
  String pie =   "/" + indexString + ".bmp ";
  char character[pie.length()];
  pie.toCharArray(character, pie.length());
      delay(20);
